...
experiment_title = "multiple_weak_signals"
...
with torch.inference_mode():
    ...
    for no_rvs in random_pc_number_of_rvs:            
        for seed in seeds:
            ...
            pc_depth = 2 * no_rvs + 1
            pc = generate_random_pc_binary(num_variables=no_rvs, max_depth=pc_depth, leafnode=random_pc_leaftype, leaves_parameters=random_pc_leaves_parameters)
            pc.is_valid()
            pc_tag = f"random_binary_pc_rvs{no_rvs}_leaf{random_pc_leaftype.__name__}"
            ...
            for dimensions_ratio in np.arange(0, 1.0+dimensions_incremental, dimensions_incremental):
                n_dims_to_inject = int(no_rvs * dimensions_ratio)
                dim_indices = np.sort(np.random.choice(list(range(no_rvs)), size=n_dims_to_inject, replace=False))
                dim_mask = torch.zeros(no_rvs)
                dim_mask[dim_indices] = 1
                dim_mask = dim_mask.bool()

                injected_samples = deepcopy(samples)
                injected_samples[np.ix_(ood_indices, dim_indices)] += signal_strength
...
results_df = pd.DataFrame(data = results, columns = ["test_side", "sum_strategy", "prod_strategy", "alpha", "pc_tag", "no_of_rvs", "seed", "id_size", "ood_size", "dimensions_ratio", "signal_strength", "nll_id", "nll_ood", "fpr", "tpr", "test_valid"])
