...
from src.models.nodewise.utils.get_default_circuits import generate_random_pc, generate_random_pc_binary
...
experiment_title = "gaussian_circuits_large"
results_dir = "./src/experiments/synthetic_only_circuits/results/"
seeds = [1, 3, 7, 11, 47]

# configuration for experiment
random_pc_number_of_rvs = 16
random_pc_depth = 8
random_pc_leaftype = Gaussian
random_pc_shift_mean_value = 2
n_samples = 10000
alphas = [0.05,]
...
# create random circuit and check for validity
pc0 = generate_random_pc_binary(num_variables=random_pc_number_of_rvs, max_depth=random_pc_depth, leafnode=Gaussian)
pc0.is_valid()
pc0_tag = f"random_pc_rvs{random_pc_number_of_rvs}_d{random_pc_depth}_leaf{random_pc_leaftype.__name__}"

# create other circuit by shifting the means of the first one
pc1 = deepcopy(pc0)
leaves = pc1.get_nodes_by_type(Gaussian)
for leaf in leaves:
    leaf.mean = torch.nn.Parameter(leaf.mean + random_pc_shift_mean_value, requires_grad=True)
pc1.is_valid()
pc1_tag = f"random_pc_shifted_mean_by_{random_pc_shift_mean_value}"
