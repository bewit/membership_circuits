...
results_dir = "./src/experiments/unsupervised_ood_detection/results/missing_data/"
...
missing_data_ratios = [0.5, 0.8]
missing_data_partitions_per_ratio = 10
...
for file_name in file_names:
    ...
    test = deepcopy(train)
    test_torch = torch.tensor(train_torch.detach().cpu())
    ...
    for seed in seeds:
        ...
        for rdc, minstr in hyper_parameters:
            ...
            for missing_data_ratio in missing_data_ratios:
                
                scopes = list(range(test.shape[1]))
                missing_data_no = int(missing_data_ratio * len(scopes))
                rng = np.random.default_rng()
                all_missing_data_scopes = []
                for i in range(missing_data_partitions_per_ratio):
                    sampled_scopes = rng.choice(scopes, size=missing_data_no, replace=False)
                    # if np.isin(sampled_scopes, all_missing_data_scopes):
                    #     i -= 1
                    #     continue
                    all_missing_data_scopes.append(sampled_scopes) 


                for mm, missing_data_scopes in enumerate(all_missing_data_scopes):


                    try:
                        
                        test_torch_missing_data = test_torch.detach().clone()
                        test_torch_missing_data[:, missing_data_scopes] = torch.nan

                        lls  = pc.log_pdf(test_torch_missing_data, write_cache=True)
                        ...
                        for test_strategy in test_strategies:
                            ...
                            pvalues = pc.test_membership_distribution(data=test_torch_missing_data, test_strategy=test_strategy)
    ...
    results_df = pd.DataFrame(data = results, columns = ["dataset", "seed", "test_side", "sum_strategy", "prod_strategy", "alpha", "id_size", "ood_size", "missing_data_ratio", "partition_no", "rdc", "minstr", "nll", "threshold_method", "fpr_densities", "tpr_densities", "auroc_densities", "fpr_testing", "tpr_testing", "auroc_testing", "test_valid"])
