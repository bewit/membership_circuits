...
results_dir = "./src/experiments/unsupervised_ood_detection/results/umap/"
data_dir = "./data/unsupervised_outlier_detection/"
models_dir = "./src/experiments/unsupervised_ood_detection/models/"
plots_dir = "./src/experiments/unsupervised_ood_detection/plots/umap/"
...
test_strategy = TestStrategy(alphas[0], test_sides[0], ArithmeticMean(), HarmonicMean())
...
for file_name in file_names:
    ...
    for seed in seeds:
        ...
        for rdc, minstr in hyper_parameters:
            try:
                ...
                samples = pc.sample(n=train.shape[0]).detach().cpu().numpy()
                samples_labels = np.array(["Sampled Data"] * samples.shape[0]).reshape(-1, 1)
                samples_labeled = np.append(samples, samples_labels, axis=1)

                data_labels = np.array(["Train Data"] * train.shape[0]).reshape(-1, 1)
                data_labeled = np.append(train, data_labels, axis=1)

                umap_data_labeled = np.concat([samples_labeled, data_labeled], axis=0)
                umap_data_raw = np.concat([samples, train], axis=0)
                reducer = umap.UMAP()
                embedding = reducer.fit_transform(umap_data_raw)
                mapper = reducer.fit(umap_data_raw)


                p = umap.plot.points(mapper, labels=umap_data_labeled[:, -1], color_key_cmap="coolwarm", alpha=transparence, width=300, height=300)
                p.texts[0].set_visible(False)
                # p = umap.plot.diagnostic(mapper, diagnostic_type="pca")
                # umap.plot.show(p)

                # plt.scatter(embedding[:, 0], embedding[:, 1])
                fpr_text = str(round(fpr_testing, 4)).replace('.', '')
                fpr_text_2 = round(fpr_testing*100, 2)
                rdc_text = str(rdc).replace('.', '')
                minstr_text = str(minstr).replace('.', '')
                transp_text = str(transparence).replace('.', '')
                plot_filename = plots_dir + experiment_title + f"umap_std_{dataset_name}_r{rdc_text}_m{minstr_text}_fpr{fpr_text}_t{transp_text}.pdf"
                plt.title(f"{dataset_name.capitalize()}: {fpr_text_2:.2f}% FPR")
    ...
    # results_df = pd.DataFrame(data = results, columns = ["dataset", "seed", "test_side", "sum_strategy", "prod_strategy", "alpha", "id_size", "ood_size", "rdc", "minstr"])
